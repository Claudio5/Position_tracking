...
* @param  pxL3GFilterInitStruct : pointer to a L3GFilterInit structure that will
*         contain the configuration setting read from the L3GX registers.
* @retval None
*/
...
void L3gxReadFifo(float* pfData, uint8_t cDataToRead)
{
  ...
  /* Read the register value to recover sensitivity and endianness*/
  L3gxByteRead(&reg,L3G_CTRL_REG4);
  ...
  else
  {
    ...
    for(uint16_t i=0 ; i<cDataToRead*3 ; i++)
    {
      aux=(((uint16_t)pcBuffer[cDataToRead*6-j-2]<<8)+(uint16_t)pcBuffer[cDataToRead*6-j-1]);
      pfData[cDataToRead*3-i-1] = (float)(*(int16_t*)&aux)/fSensitivity;
      j+=2;
    }
  } 
...
* @param pxFifoInit: pointer to the Fifo initialization structure.
