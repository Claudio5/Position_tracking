...
/**
 * @brief Macro for the absolute value.
 */ 
...
/**
 * @brief Value for the Epsilon Threshold.
 */   
#define EPS 110.0f    

/**
 * @brief Static array in float for min values of Magnetometer Data.
 */ 
static float s_fMagMin[3];

/**
 * @brief Static array in float for max values of Magnetometer Data.
 */ 
static float s_fMagMax[3];

/**
 * @brief Static variable mask of 6-bits so defined [0 0 0 MIN_Z_REACH MIN_Y_REACH MIN_X_REACH MAX_Z_REACH MAX_Y_REACH MAX_X_REACH] .
 */ 
...
void iNEMO_TiltedCompass(float* pfMagXYZ, float* pfAccXYZ, float* pfRPH)
{
  ...
  pfRPH[2] = -atan2(fTiltedY,fTiltedX);
  
  /* return the roll and pitch angles */
  pfRPH[0]  = atan2(fSinRoll,fCosRoll);
  pfRPH[1] = atan2(fSinPitch,fCosPitch);
